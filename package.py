...
def overrideSetting(file, now):
    with open(file['settingFile'], 'r', encoding='utf-8') as f:
        lines = f.readlines()
        with open(file['settingFile'], 'w', encoding='utf-8', buffering=1) as f_w:
            for line in lines:
                if "version: " in line:
...
def packaging():
    ...
    for file in FileList:
        ...
        if PackageArgs['distpath']:
            ...
            tmp = OutPath + '\\' + file['name'].replace('.py', '.exe')
            if os.path.exists(tmp):
                if TypeNum == 4:
                    subprocess.run('move ' + tmp + ' ' + OutPath + '\\"' + file['outName'] + '_' + Computer_Digits + '_' + now + '.exe"', shell=True)
                else:
                    subprocess.run('move ' + tmp + ' ' + OutPath + '\\"' + file['outName'] + '_' + Type[TypeNum] + Computer_Digits + '_' + now + '.exe"', shell=True)
...
if __name__ == "__main__":
    ...
    # PackageArgs['distpath'] = r'F:\PythonAPP_Package\Wiretap-Tunnel'
    # PackageArgs['distpath'] = r'Y:\work_2\PythonApp_Package\Wiretap-Tunnel'
